...
def reset_game():
    player = Player()
    enemies = [Enemy(x, 50) for x in range(50, WIDTH - 50, 60)]
    bullets = []
    power_ups = []
    boss = None
...
while running:
    ...
    if game_state == MENU:
        ...
        exit = font.render("Press Q to Exit", True, WHITE)
        ...
        font = pygame.font.Font(None, 36)
        text = font.render(f"ROUND: {round_num}  LEVEL: {level}", True, WHITE)
        screen.blit(text, (10, 10))

        # Display timers
        speed_text = font.render(f"Speed Timer: {player.speed_timer // 60}", True, WHITE)
        bullets_text = font.render(f"Bullets Timer: {player.bullet_timer // 60}", True, WHITE)
        ...
        lost = font.render("You've Lost", True, WHITE)
        try_again = font.render("Press R to Try Again", True, WHITE)
        exit = font.render("Press Q to Exit", True, WHITE)
